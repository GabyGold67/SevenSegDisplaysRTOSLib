/*
  twoDisplaysExampleESP32.ino - Example file to demonstrate SevenSeg74HC595 class use with two displays simultaneously
...
const uint8_t dsp2Dio {13}; // Board pin # of ESP32 WROOM-32 uPesy dev Board
const uint8_t dsp2Rclk {12}; // Board pin # of ESP32 WROOM-32 uPesy dev Board
const uint8_t dsp2Sclk {14}; // Board pin # of ESP32 WROOM-32 uPesy dev Board
...
// RCLK
// DIO
SevenSeg74HC595 myLedDispOne(dsp1Sclk, dsp1Rclk, dsp1Dio);
SevenSeg74HC595 myLedDispTwo(dsp2Sclk, dsp2Rclk, dsp2Dio);
...
void loop()
{
  ...
  if ((xTaskGetTickCount()/portTICK_PERIOD_MS - timer) >= testTime)
  {
    ...
    if (testNum == firstTest){
      //Attaching the display refresh to the RTOS timer Daemon trough the begin() method
      myLedDispOne.begin();
      myLedDispTwo.begin();
    }
    switch (testNum)
